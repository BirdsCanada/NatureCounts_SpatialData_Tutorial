...
-   Transform and combine raster data objects

-   Reclassify raster data

-   Perform raster algebra and calculate summary statistics using local, global, and zonal functions
...
```
# 7.1 Raster Data Transformations
...
```{r}
elevation_rast <- rast("path/to/your/mauricie_mask.tif")
elevation_rast
```
...
# 7.3 Raster Algebra & Summary Statistics  
...
In Chapter 4, Chapter 5, and Chapter 6 you extracted elevation, land cover, and NDVI values over bird observation sites across La Mauricie National Park. These data were uploaded to the [Git repository data folder](https://github.com/BirdsCanada/NatureCounts_SpatialData_Tutorial/tree/d83eed714373048c8f9fdb4ea4a17f1fd4bef3f7/Data) and the [Google Drive data folder](https://drive.google.com/drive/folders/1gLUC6fROl4kNBvTGselhZif-arPexZbY?usp=sharing) for your convenience. 
...
We can summarize the combined dataset to explore possible trends.

Create a boxplot of elevation grouped by landcover class

```{r}
ggplot(mauricie_data, aes(x = as.factor(landcover), y = elevation)) +
  geom_boxplot() +  # Boxplot of elevation by landcover class
  geom_jitter(width = 0.2, alpha = 0.5) +  # Add individual points with jitter
  labs(title = "Boxplot of Elevation by Landcover Class",
       x = "Landcover",
       y = "Elevation") +
  theme_minimal()
```
...
# Calculate mean NDVI by SiteCode
...
# Plot species richness vs. mean NDVI
ggplot(species_ndvi_data, aes(x = min_ndvi, y = species_richness)) +
  geom_point() +
  labs(title = "Bird Species Richness vs. Mean NDVI in La Mauricie National Park",
       x = "Min NDVI",
       y = "Bird Species Richness") +
  theme_minimal()
```
Congratulations! You completed Chapter 7 - Raster Summary Tools. In this chapter, you successfully transformed and combined raster data, performed raster algebra and calculated local, global and zonal summary statistics. In [Chapter 8](08-AnalysisTools.Rmd), you can explore additional NatureCounts auxiliary data from nc_query_tables and analysis tools.
