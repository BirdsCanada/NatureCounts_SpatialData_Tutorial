...
landcover_rast
```
You'll notice that the resolution, extent, and CRS of the elevation and land cover raster layers do not match which prevents us from computing these layers together.

# 7.2 Raster Data Transformations
...
plot(mauricie_boundary, add = TRUE, border = "red", col = NA, lwd = 2)
```
# 7.3 Raster Reclassification

When communicating our data, it might be useful to reclassify a range of raster values into smaller set of classes. In this process, a range of values are binned to a new (categorical) value. Reclassifying rasters is easily achieved using `terra` and the `classify()` function.
...
# 7.4 Combine Raster Data 

Multiple single-layer SpatRaster with the same CRS, extent, and resolution, can be combined into a single multi-layer SpatRaster with `c()` function.

```{r}
mauricie_rast <- c(elevation_rast_proj, landcover_rast_resample)
mauricie_rast
```
You can subset layers of a SpatRaster with the $ or [[]] operators. 
...
```
# 7.5 Calculate Raster Summary Statistics
