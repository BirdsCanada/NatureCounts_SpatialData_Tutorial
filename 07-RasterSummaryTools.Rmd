...
# 7.4 Explore Patterns in Species Abundance
...
ggplot(species_rank_landcover, aes(x = as.numeric(rank), y = total_abundance, color = landcover)) +
  ...
  theme_minimal() +
  labs(
    title = "Abundance for High Rank Species across Broadleaf and Needleleaf Forests",
    x = "Species Rank",
    y = "Abundance",
    color = "Landcover"
  ) +
...
```{r}
# Create NDVI ranges with numeric labels
...
# Calculate species abundance for each NDVI range 
...
# Plot relative species abundance by each NDVI range 
