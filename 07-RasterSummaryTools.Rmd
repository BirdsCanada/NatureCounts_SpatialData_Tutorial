...
> In Chapter 6, you downloaded satellite imagery from the Copernicus SENTINEL-2 mission and calculated spectral indices (NDWI, NDVI) over [La Mauricie National Park](https://parks.canada.ca/pn-np/qc/mauricie/nature) to combine with NatureCounts data from the [Quebec Breeding Bird Atlas (2010 - 2014)](https://naturecounts.ca/nc/default/datasets.jsp?code=QCATLAS2PC&sec=bmdr). In this chapter, you will combine and reclassify raster data, calculate summary statistics, and create data summaries and visualizations.

**This Chapter build on the previous, and uses the the National Park boundary and NatureCounts data from downloaded in section 4.1 Data Setup from [Chapter 4: Elevation Data](04-ElevationData.Rmd)**. **Raster data were dowloaded in [Chapter 4: Elevation Data](04-ElevationData.Rmd) and [Chapter 5: Land Cover Data](05-LandcoverData.Rmd) which are also required for this lesson. For quick access, these layers are available for download via the [Git repository](https://github.com/BirdsCanada/NatureCounts_SpatialData_Tutorial/tree/d83eed714373048c8f9fdb4ea4a17f1fd4bef3f7/Data) or [Google Drive](https://drive.google.com/drive/folders/1gLUC6fROl4kNBvTGselhZif-arPexZbY?usp=sharing) data folders. If you wish to gain experience in how to download, process, and save environmental layers yourself, return to the earlier chapters of this tutorial series.**
...
-   Create and visualize NatureCounts data summaries using environmental covariates: [Summary Tools: Species Abundance](#7.4SummaryTools)
...
Raster data is stored as a SpatRaster object and can have one or multiple layers. To open a SpatRaster, we can use the `rast()` function from the `terra` package. In [Chapter 4: Elevation Data](04-ElevationData.Rmd), you created a mask of La Mauricie National Park using single-layer DTM elevation data. Read this file from your directory and call it `elevation_rast`: ###Could we put the in the root folder in Google drive and also in the tutorial. We want to keep this simple \###
...
```{r}
mauricie_rast$elevation ####Error: [subset] invalid name(s)
...
landcover_summary <- mauricie_data %>%
  group_by(landcover_class) %>% ###LANDCOVER NOT IN DATASET. Should it be? THe names of the landcover class would be more useful than the numeric value. 
  summarize(
...
Group the NatureCounts data by species and rank them in order of abundance. ###THIS WON'T WORK. Landcover not in dataset. \###
...
###COULD WE PROVIDE TEXT HERE?? What is this example going to show the user?###
...
Plot species richness and minimum NDVI grouped by SiteCode. ###THERE IS AN ISSUE WITH SITE CODE IN THIS DATASET. They are not unique to each lat and long###
