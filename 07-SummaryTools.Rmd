...
ggplot(hist_data, aes(x = value)) +
  ...
  labs(title = "Multipanel Histogram of Environmental Variables",
       ...
       y = "Frequency")
```
##Lets delete since this is redundant to creating a historgram which can also be used to assess distribution##
...
##Since we explore relationship between each response and the predictor below, we probably don't need this##
...
events_matrix <- events_matrix %>% 
  group_by(SiteCode, geometry) %>% 
  mutate(point_id = cur_group_id()) %>% ###Are we positive that this point id will align with the point ID in the environmental data##
  ungroup()
```
...
Users may wish to explore these data using simple statistics, like logisitic regression, and create response curves showing the relationship between covariates and the probability of a species being present.
...
<!-- Pivot the data for plotting. ###I don't think this code chunk is working or needed### -->

<!-- <!-- ```{r} --> -->
<!-- <!-- kinglet_PA_summary_long <- kinglet_PA_summary %>% --> -->
<!-- <!--   pivot_longer(cols = c(total_presence, total_absence),  --> -->
<!-- <!--                names_to = "category",  --> -->
<!-- <!--                values_to = "count") --> -->
<!-- <!-- ``` --> -->
