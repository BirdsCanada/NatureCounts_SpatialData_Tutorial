...
```{r}
ggplot(species_rank, aes(x = as.numeric(rank), y = total_abundance, color = factor(point_id))) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  theme_minimal() +
  labs(
  ...
  scale_color_manual(values = RColorBrewer::brewer.pal(n = length(unique(species_rank$point_id)), "Set3"))
```

# 7.2 Species Richness

Calculate the species richness (number of species) for each `point_id`.

```{r}
mauricie_sp_richness <- mauricie_data %>%
