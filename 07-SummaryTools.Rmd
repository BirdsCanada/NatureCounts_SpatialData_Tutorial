...
By the end of **Chapter 7 - Summary Statistics ###Is this the title of is the title "Summary Tools? Please correct here and in previous chapters if needed###**, users will know how to:
...
In [Chapter 4](04-ElevationData.Rmd), [Chapter 5](05-LandcoverData.Rmd), and [Chapter 6](06-SatelliteImagery.Rmd) you extracted elevation, land cover, and NDVI values, respectively, at surveys sites across La Mauricie National Park. These data were uploaded to the [Google Drive data folder](https://drive.google.com/drive/folders/1gLUC6fROl4kNBvTGselhZif-arPexZbY?usp=sharing) for your convenience.

Let's download all the environmental covariates and join them to a common dataframe.

```{r, message = FALSE}
# List the dataframes
env_covariates <- list.files(path = "data/mauricie/env_covariates", ###we had users save these to thier working directory. Could you please provide the code that is compatible with this###
                             pattern = "\\.csv$", 
                             full.names = TRUE)

###There is an issue with your landscape data file in the maurice data folder. Please run all code to check that it works.###
...
Combine the environmental covariates with the NatureCounts data. ###Why do we combine the NatureCounts data with the covarites here, and then pull them apart, and then put them back together gain. Please fix. \###
...
Drop all columns except the `point_id` and environmental variables and group all rows together by point_id.
...
Join the `biodiversity_count` dataframe. Cleanup variable names. ###You joined these above. Do we need to do this again here.###

##This code breaks because there are duplicates in the landscape_metric_df.csv. Please check all code below to ensure it works \###
