...
By the end of **Chapter 7 - Summary Tools**, users will know how to:
...
###I notice gaps in the n_species histogram. Is this because of binning###
...
ggplot(enviro_data_df, aes(x = pland_mixed_forest, y = n_species)) +
  geom_point() +
  geom_smooth(method = "lm", col = "blue") +
  labs(title = "Relationship between % Mixed Forest and Species Richness") 
...
##Might remove this example since the results don't make sense bioogically. Can't sum the species richness##
...
elevation_summary <- enviro_data_df %>%
  ...
  summarize(spec_richness = sum(n_species, na.rm = TRUE), .groups = "drop")  ###You can't sum the species richness or you are double counting the species. You should count the number of unique species in each elevation class###
...
Create a biplot of the the percent land class variables.
...
ggplot(df_pca, aes(x = PC1, y = PC2, color = cluster)) +
  geom_point(size = 3) +
  labs(title = "K-Means Clustering of Species Richness and % land class metrics",
