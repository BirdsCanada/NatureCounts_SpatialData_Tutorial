...
*Example 1*: You would like to visualize the summer spatio-temporal distribution of Cedar Waxwing's for each survey year of the Maritimes Breeding Bird Atlas (2006-2010).

The [NatureCounts Introductory R Tutorial](#link) is where you should start if you're new to the `naturecounts` R package. It covers how to view, filter, manipulate, and visualize NatureCounts data. We recommend reviewing this tutorial before proceeding.
...
```{r}
cedar_waxwing <- nc_data_dl(collections = "MBBA2PC", species = 16330, username = "dimimarkou", info = "spatial_data_tutorial")
```
...
```{r}
cedar_waxwing_sf <- sf::st_as_sf(cedar_waxwing,
...
leaflet() %>%
  addTiles() %>%
  addCircleMarkers(data = cedar_waxwing_sf, 
                   radius = 2, 
                   color = "black", 
                   stroke = FALSE, 
                   fillOpacity = 0.8)
...
```{r}
ggplot(data = cedar_waxwing_sf) +
  annotation_map_tile(type = "osm", zoom = 6) +
  geom_sf(aes(color = factor(survey_year)), size = 1, alpha = 0.8) +
...
# Plotting
ggplot(data = cedar_waxwing_sf) +
  geom_sf(aes(color = survey_year)) +  # Use color to distinguish years
  facet_wrap(~ survey_year) +  # Create a panel for each survey year
  labs(title = "Cedar Waxwing Observations by Survey Year",
       color = "Survey Year") +
  theme_minimal() +
...
The following sections (1.3 - 1.7) comprise a sequential methodology that will help the user visualize NatureCounts data using [Key Biodiversity Areas (KBAs)](https://kbacanada.org/about/) and [Priority Places for Species at Risk](https://environmental-maps.canada.ca/CWS_Storylines/index-ca-en.html#/en/priority_places-lieux_prioritaires) maps.
...
*Example 2*: You want to assess the spatial distribution of Wood Ducks across the KBA's in Ontario using Ontario Breeding Bird Atlas data.
...
```{r}
ontario_kba <- sf::st_read("Data/KBA/ProvOntario_KBA/ontario_kba.shp")
```
...
Let's search NatureCounts for the ONATLAS3PC dataset to assess Wood Duck distribution across Ontario KBA's:
...
After geoprocessing our data in R, we can write out any sf objects to Shapefiles on a disk:

```{r}
st_write(wood_ducks_kba,
         "Data/wood_ducks_kba.shp",
         driver = "ESRI Shapefile",
         delete_layer = TRUE)
```

where the argument delete_layer = TRUE is used to overwrite a file if needed.
...
```{r}
gdb_path <- "Data/Priority Places/PriorityPlaces.gdb"
```
