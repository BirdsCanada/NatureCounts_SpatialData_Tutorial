...
We can create a basic plot using the function plot() combined with st_geometry()

```{r}
plot(st_geometry(lake_superior_kba), axes = TRUE,
                                     col = "NA", border = "red")
```

Or visualize the KBA with an interactive map like we did before:
...
We can then convert our NatureCounts data into a spatial object. To do so, we can deploy the `st_as_sf` function and specify the coordinate reference system (CRS) to match our **lake_superior_kba** projection.
...
Our KBA sf object is stored with World Geodetic System 1984 (WGS 84) coordinates, EPSG = 4326. This is how we can convert our **ontario_bbird_survey** to an sf object using the same CRS:
...
For example, `st_transform()` can be applied like so to ensure that the CRS of our spatial objects match:
```{r}
ontario_bbird_survey <- st_transform(ontario_bbird_survey, crs = st_crs(lake_superior_kba))
```
...
```{r}
ls_bbird_survey <- sf::st_intersection(lake_superior_kba, ontario_bbird_survey)
```
```{r}
ls_bbird_survey <- ontario_bbird_survey[st_within(ontario_bbird_survey, lake_superior_kba), ]
```



After geoprocessing our data, we can write out any sf objects to Shapefiles on a disk.

```{r}
st_write(ls_bbird_survey,
         "data/ls_bbird_survey.shp",
