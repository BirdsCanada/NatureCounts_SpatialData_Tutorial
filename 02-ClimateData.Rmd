...
In this tutorial, you will use NatureCounts and climate data to explore possible patterns in Whooping Crane observations, annual mean temperature, and annual mean precipitation within the Wood Buffalo National Park, the province of Alberta, and beyond. This tutorial will bind using spatio-temporal maps.
...
Grouping the Whooping Crane data might provide more meaningful insight on species occurrence and how it varies across time and space. First, it is important that you create date and doy columns and ensure that the ObservationCount column is in the correct `numeric` format:
...
You can then summarise species occurrence by SiteCode and date while keeping the coordinate information for each site:
...
Lastly, you can summarize the NatureCounts data once more to get the annual total counts of Whooping Cranes, say between 1985 and 2011:
...
Finally, to summarize the weather data by calculating the annual mean summer temperature and precipitation:
...
Convert the species_occurences_summary, which summarises species occurrence by SiteCode, into a spatial object:
...
Filter the data to only include observations made post 1970:
...
ggplot(data = bioclim_data, aes(x = bio1, y = total_count)) +
  ...
  labs(title = "Total Count and Annual Mean Temperature at each Site",
