...
-   Process land cover rasters, including cropping and masking: [5.2 Spatial Extents - Crop and Mask](#5.2Crop-Mask)
...
# 5.2 Raster Color Maps {#5.3ColorMaps}
...
# 5.3 Point Buffers {#5.4PointBuffers}
...
Use the `st_buffer` function to generate 3 km diameter circular neighborhoods centered at each observation location, filtered for the 2014 survey year.
...
Spatial patterns can be observed using different representation schemes including categorical classification, graph-based representation or gradient classification.

**Landscape metrics** help us quantify spatial patterns in landscape composition. This refers to the proportion of cover types present from each class per specified area. Landscape metrics can therefore help assess species distribution patterns between areas based on how they partially describe the landscape.

The `landscapemetrics` package can be used to calculate some common metrics:
...
```{r}
plot(mauricie_birds_buffer_mask)
```

Use the `calculate_lsm()` function to calculate `pland` and `ed` metrics within each neighborhood and for each represented class.

```{r}
metrics_class <- calculate_lsm(mauricie_birds_buffer_mask,
...
for (i in seq_len(nrow(mauricie_birds_buffer))) {
  ...
  # Crop and mask the single-year landcover data
...
```{r}
# Convert lsm to wide format with landcover classes
lsm_wide <- lsm |> 
  # Fill missing classes with zeros
  complete(nesting(Locality, survey_year),
           class = lc_classes$class,
           metric = c("ed", "pland"),
           fill = list(value = 0)) |> 
  # Merge landcover class labels
  ...
  # Arrange by locality and survey year
  arrange(Locality, survey_year)
```

# 5.5 Map & Extract Land Cover Data {#5.4Map-Extract}
