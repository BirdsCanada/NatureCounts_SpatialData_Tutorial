...
-   Calculate landscape metrics, including PLAND, NP, and ED ###Please specific what these acronyms mean###: [Landscape Metrics](#5.4LandscapeMetrics)
...
# writeRaster(landcover_mask, output_dir, overwrite = TRUE) #####Error: [writeRaster] cannot guess file type from filename
```
...
lsm_sitesum_df <- left_join(mauricie_site_summary, landscape_metrics_df, by = "point_id") ###You dropped the point ID when saving above. This breaks the code. Please test all code to ensure it works.###
