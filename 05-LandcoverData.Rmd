...
-   Assign land cover class labels and create a custom plot: [Land Cover Class Labels](#5.2ClassLabels)
...
# 5.2 Land Cover Class Labels {#5.2ClassLabels}
...
list(unique(landcover_mask_df$Class)) # determine unique land cover class values in the cropped dataset

land_cover_data <- landcover_mask_df %>%
  arrange(Class) %>%  # Sort by land_cover_value
  mutate(landcover_name = case_when(
    Class == 1 ~ "needleleaf",
    Class == 5 ~ "broadleaf",
    Class == 6 ~ "mixed forest",
    Class == 8 ~ "shrubland",
    Class == 10 ~ "grassland",
    Class == 14 ~ "wetland",
    Class == 16 ~ "barren",
...
lc_classes <- land_cover_data %>% select(Class, landcover_name) %>% distinct()

lsm <- inner_join(mutate(lsm, class = as.factor(class)), lc_classes, by = c("class" = "Class"))
```
...
# 5.5 Join Land Cover Data {#5.5Map-Extract}
...
landscape_metrics_df  <- mauricie_birds_sf %>%
  ...
  left_join(lsm_landscape, by = "point_id") # append landscape-level metrics

landscape_metrics_df  <- landscape_metrics_df  %>% select(point_id, longitude, latitude, record_id, species_id, english_name, SiteCode, survey_year, survey_month, survey_day, ObservationCount, ed, np)
```
...
```{r}
# write.csv(landscape_metrics_df , "data/mauricie/landscape_metrics_df.csv", row.names = FALSE)
```
