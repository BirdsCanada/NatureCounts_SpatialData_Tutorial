...
par(mfrow = c(1, 1))
```
OPTIONAL: To write the masked raster to your disk, you can use the writeRaster() function from terra. This raster will be useful to complete Chapter 7: Raster Summary Tools. You must complete the above sections and save this raster to proceed with Chapter 7 unless you're using your own data.
...
Examine the contents of the cropped land cover raster. The `freq()` function describes the pixel count per land cover class represented in the region.

```{r}
freq(landcover_mask)
```
Examine the categories of the cropped land cover raster. The `cats()` function displays the color table that associates each class with a distinct color. Let's save this to our environment as a dataframe called **landcover_rgb** using the `as.data.frame()` function.

```{r}
landcover_rgb <- as.data.frame(cats(landcover_mask))
landcover_rgb
```

Filter the dataframe to only include those classes represented in the cropped land cover raster. To assign the appropriate labels to each land cover class, refer to the [Class Index](https://open.canada.ca/data/en/dataset/ee1580ab-a23d-4f86-a09b-79763677eb47/resource/b8411562-49b7-4cf6-ac61-dbb893b182cc)) and use the `mutate()` function after filtering for the relevant classes and tidying-up the attribute names.
...
Next, merge the two dataframes by performing a `left_join()`. The grouping variable is case sensitive. 
...
mauricie_birds <- read_csv("data/mauricie/mauricie_birds_df.csv") 
```
...
```{r}
mauricie_birds_sf <- sf::st_as_sf(mauricie_birds,
                        coords = c("longitude", "latitude"), crs = 4326)

mauricie_boundary <- st_transform(mauricie_boundary, crs = st_crs(mauricie_birds_sf)) # match the CRS
```

Summarize the NatureCounts data for mapping and convert it to SpatVector format.
...
# Extract the landcover class ID for each site
landcover_sites_sf <- terra::extract(landcover_mask, vect(mauricie_birds_sf), bind = TRUE)
```
...
```{r}
landcover_sites_df <- terra::as.data.frame(landcover_sites_sf) %>% 
 select(record_id, landcover_class = Class) 
```

```{r}
# Combine with NatureCounts dataset
mauricie_birds <- mauricie_birds %>%
  left_join(landcover_sites_df, by = "record_id") %>% # Join by land cover class
