...
###CAN WE MAKE CROP AND MASK THERE OWN LEARNING OBJECTING AND SUBSECTION SAME AS CHAPTER 4. IT IS OK IF THEY ARE THE SAME SET OF SKILLS TWICE###
...
# 5.2 Raster Color Maps {#5.2ColorMaps} ###Note certain we need the colour map. we can just have ggplot assign random colours? If this is the case, we just want to assign landcover values to thier names
...
###NOTE THIS MAP LOOKS REALLY WONGKY ON MY END \### ##Why do we assign the color ourselves. I think it would be easier if you let ggplot just pick random colour for each landcover_class. This would elimiate the need to keep the landcover_rgb dataframe which I think is somewhat confusing \###
...
Create an `sf` object from the NatureCounts data, summarized by distinct point count locations (###ISSUES WITH THIS DATA - the point count locations do not have unique identifier. This means we need to get the point information based on Lat an Long and associate it back with the original dataframe#####) and survey year. Because our land cover data is from 2015, we filter the NatureCounts data for the most recent survey year (2014).
...
```{r}
mauricie_birds_local$point_id <- paste0("point_id_", seq_len(nrow(mauricie_birds_local)))
```
...
In landscape ecology, the composition and configuration of land types tells us what habitat is available and how its distributed spatially. **Landscape metrics** help us characterize these spatial patterns across landscapes ###WITHIN A BUFFER or specific region?
...
For each point buffer, crop & mask the land cover data, calculate PLAND and ED, and create a dataframe containing the class-level landscape metrics.
...
for (i in seq_len(nrow(mauricie_birds_buffer))) {
  ...
  lsm[[i]] <- crop(landcover, buffer_i) %>%  ###KEEP THE PIPE OPERATOR CONSISTENT
    ...
    select(point_id, class, metric, value)
}
...
Calculate the mean class-level PLAND and ED per Locality and convert to wide format. ###Does this need to be the mean now taht I have made this the point_id???####

```{r}
# Calculate mean class-level metrics per Locality and convert to wide format
lsm_wide <- lsm %>%
  group_by(point_id, landcover, metric) %>% 
...
Compare the PLAND and ED values across each count location.

```{r}
# Filter for pland metric and calculate mean values
pland_means <- lsm %>%
  filter(metric == "pland") %>%           # Filter only pland values
  group_by(point_id, landcover) %>%       # Group by Locality and landcover
...
ggplot(pland_means, aes(x = landcover, y = mean_pland, fill = landcover)) +
  ...
  facet_wrap(~ point_id, scales = "free_y") + # One panel per Atlas Square
  labs(
...
###I DELETED A CHUNK OF CODE HERE. I wasn't certain it was needed and wanted the user to get to the NatureCounts data integration faster.###
...
####DELETED CODE. redundant with the previous section#### Create a spatial dataframe from the NatureCounts data and join the unique point_id from the buffer geometry to the NatureCounts data by latitude and longitude using `st_join`.
...
ggplot() +
  ...
  # Custom color scale for landcover ###Sample comments as above. Can we have ggplot assign random colour and remove this step?
  scale_fill_manual(
    values = setNames(
      rgb(landcover_rgb$red, landcover_rgb$green, landcover_rgb$blue, maxColorValue = 255),
      landcover_rgb$landcover
...
##DO WE NEED TO KNOW THIS?I WOULD SAY WE COULD END AT THE BUFFER TOOL SINCE THIS IS GOING TO BE MOST USEFUL### Extract the land cover class id for each point id and append it to `mauricie_birds_df`.
