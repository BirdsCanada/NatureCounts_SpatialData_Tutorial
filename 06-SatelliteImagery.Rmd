...
```{r}
bands <- list.files(path = "path/to/your/directory", 
...
ndvi_values <- terra::extract(mauricie_sl_ndvi, vect(mauricie_birds_sf)) # Extracts the NDVI values for each point
```
...
```{r}
ndvi_values_df <- terra::as.data.frame(ndvi_values) %>% 
...
mauricie_birds <- mauricie_birds %>%
  left_join(ndvi_values_df, by = "record_id") # Join by land cover class
```
