...
# Chapter 6: Calculating NDWI and NDVI using SENTINEL2 imagery ####Can we shorten this title. "Satellite Imagery" is more in line with the other chapter titles.
...
###The next several steps are unclear why we are doing them? Why do we need the water mask and water mode? Are any of these steps redundant? Can some of them be removed. Otherwise, it needs to be made clear why they are needed###
...
Apply the water ####mode?? mask to `mauricie_sl`.
...
Summarize the NatureCounts data for mapping. ###Could we make this species diversity like the previous examples and map the dot sizes based on this###
...
To map ###species diversity and NDVI data:
...
Extract NDVI values for each survey site and append it to `mauricie_birds`.
...
Combine the NDVI dataframe with the NatureCounts dataframe. ###We don't do anything with this output? Can we create a plot similar to previous chapters###
