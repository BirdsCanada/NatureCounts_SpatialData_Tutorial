...
1)  Click the **Create an area of interest** ##This is a pentagon on the upper righthand side of my screen## tab and select the **Upload a file to create an area of interest** ##The is an upward arrow on the pop out submenu## option. Upload the KML National Park boundary file we created in Chapter 4, section 4.1. ##or grab it from the folder (which will need to be moved).

2)  In the **Search** tab (top, left side of window), select SENTINEL-2 and the time range that overlaps your data to retrieve the following image:

##I can't get this to work. Perhaps if we don't expect users to get the same data, they could just search by month and find a clear day? 
