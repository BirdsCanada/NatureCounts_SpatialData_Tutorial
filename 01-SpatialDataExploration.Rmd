...
# 1.0 Learning Objectives {#1.0_Objectives}

By the end of **Chapter 1 - Spatial Data Exploration**, users will know how to:

-   Distinguish between types of spatial data (vector vs raster)(#1.1Types1.1) ###Similar to other chapter, please link these to their respective subheadings. Keep format consistent###
...
# 1.1 Spatial Data Types {#1.1Types} ###Check you have done this is all chapters
...
| `st_intersection(x, y)` - computes the geometric intersection of two or more geometries. The results is a new geometry that represents the area where the input overlap. 
| `st_crop(x, y, ..., xmin, ymin, xmax, ymax)` - creates geometry of x that intersects a specified range ###Could you made this definition a little more explicit.###
| `st_difference(x, y)` - creates geometry from x that does not intersect with y ###Same###
...
#### *Example 1* - You would like to visualize the spatio-temporal distribution of Cedar Waxwing observations in June of each survey year using data from the Maritimes Breeding Bird Atlas (2006-2010). ###You don't set up the other chapters with "Example". Could we keep the format consistent###
