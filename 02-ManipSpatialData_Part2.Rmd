...
Let's visualize average temperature over the Priority Places of Nova Scotia, New Brunswick, and Prince Edward Island, respectively. 
...
We're interested in mean_temp for the Priority Places contained within the Maritime provinces.  We'll use `dplyr` to filter based on a variable condition: 
...
Now we can join the spatial weathercan data with the Priority Place data and group the data by relevant columns before calculating the mean temperature across all stations within each polygon:
...
Clip the atlas data to include observations found within the priority places:

```{r}
cw_priori_place_obs <- sf::st_intersection(maritime_polygons, maritimes_atlas_sf)
```
Transform the CRS of our spatial data layers so that they are suitable for mapping:
...
unique_ids <- unique(mar_temp_summary$Name)
...
Plot the mean_temp and Cedar Waxwing observation data for each Priority Place using mapview:
