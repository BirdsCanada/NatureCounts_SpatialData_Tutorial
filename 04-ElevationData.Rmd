...
First, drop the geometry column and convert the NatureCounts data back to a regular dataframe.

```{r}
mauricie_birds_df <- mauricie_birds %>%
  st_drop_geometry() %>%  # Drops the geometry column
  bind_cols(
    st_coordinates(mauricie_birds) %>%  # Extract coordinates
