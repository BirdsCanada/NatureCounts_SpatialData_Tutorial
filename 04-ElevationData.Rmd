...
-   Distinguish between different types of digital elevation datasets: [4.2 Elevation Models](#4.2ElevationModels)
-   Process Digital Terrain Models (DTMs), including cropping and masking: [4.3 Spatial Extents - Crop and Mask](#4.3Crop-Mask)
...
-   Know how to access LiDAR-derived data products: [4.5 LiDAR Data Resources](#4.5LiDAR) and [4.6 Manual Data Download](#4.6LiDAR)
...
```{r}
quebec_atlas <- nc_data_dl(collections = "QCATLAS2PC", username = "testuser", fields_set = "extended", info = "spatial_data_tutorial", timeout = 500)
```
...
```{r, eval = FALSE}
# quebec_atlas <- nc_data_dl(collections = c("QCATLAS2PC", "EBIRD-CA-QC"), username = "testuser", field_set = "extended", info = "spatial_data_tutorial")
```
...
Tidyverse functions can help us summarize our data in a variety of ways. For example, here we determine the annual total count of birds for each year across all sites. To do this we use the `group_by()` function to group the observations by year, and `summarise()` to calculate and create the **annual_count** column. 
...
If you wanted to summarize total count for each species at each point count site you could adjust the pipe using `pivot_wider()`.

```{r}
mauricie_species_summary <- mauricie_birds_sf %>%
  st_drop_geometry() %>%  # drop the geometry column
  group_by(english_name, SiteCode) %>%
...
dtm_mosaic <- do.call(mosaic, dtm_list) ###Error: [mosaic] resolution does not match
...
```{r}
# output_path <- "path/to/your/folder/dtm_mask.tif"  

# writeRaster(dtm_mask, output_path, overwrite = TRUE)
```
...
plot(dtm_mask, main = "Mauricie Mask with Site Summary Points")
