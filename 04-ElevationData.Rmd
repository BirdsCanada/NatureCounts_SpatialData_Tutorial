...
mauricie_birds_df <- mauricie_birds_df %>% select(record_id, species_id, english_name, SiteCode, survey_year, survey_month, survey_day, ObservationCount, longitude, latitude) #reduce the dataframe to the columns needed for your analysis
```
...
**Note: if there are other .tiff files in your working directory they may create an error with this code.**
...
###This plot function is redundant to the one below. Delete. <!-- We can then use the base `plot()` function to visualize the terrain and boundary of La Mauricie National Park. -->
...
Now we can map the NatureCounts summary output with the and DTM data. 
...
plot(st_geometry(mauricie_site_summary), 
     ...
     cex = mauricie_site_summary$n_species_scale)      # Point size based on total count
```

Extract elevation values for each bird observation record and append it to `mauricie_birds`.
