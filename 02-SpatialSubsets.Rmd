...
# 2.0 Learning Objectives {#2.0_Lean}

By the end of **Chapter 2 - Spatial Subsetting**, users will know how to:

-   Import, reproject, and visualize spatial vector data within:
    -   [Key Biodiversity Areas (KBAs)](#2.1_KBA)

    -   [Priority Places for Species at Risk](#2.2_PP)

The data used in this tutorial are downloaded from NatureCounts, the [KBA Canada Map Viewer](https://kbacanada.org/explore/map-viewer/), and the [Priority Places - Open Government Portal](https://open.canada.ca/data/en/dataset/91219d24-e877-4c8a-8bd2-b2b662e573e0). You will save the downloaded shapefiles to a new folder (subdirectory) called `data` to keep things tidy.
...
To set your working directory, if necessary.

```{r}
setwd("path/to/your/directory")
```

To create the `data` subdirectory. This is where you will store all data used in this tutorial series.

```{r}
if (!dir.exists("data")) { # checks if "data" subdirectory exists
  print("Directory does not exist. Creating now...")
  dir.create("data") # creates "data" subdirectory
} else {
  print("Directory already exists.")
}
```

Create two additional subdirectories: one for the KBA data and the other for the Priority Places data.

```{r}
dir.create("data/kba", recursive = TRUE)
dir.create("data/priority_places", recursive = TRUE)
```
...
# 2.1 Key Biodiversity Areas (KBA) {#2.1_KBA}
...
Navigate the the [KBA Canada Map Viewer](https://kbacanada.org/explore/map-viewer/) and filter the data for Ontario using the left hand `Province/Territory` filter. Then select 'Download'. You will want to select both `csv` and `shp` for this example. Move these files to your `kba` subdirectory.

We can read in our KBA polygons using the `sf` package once it is in your subdirectory. The downloaded files were renamed for this example so you will need to change the code to match your file name.

```{r}
ontario_kba <- sf::st_read("data/kba/ontario_kba.shp")
```
...
```{r}
kba_attributes <- read.csv("data/kba/ontario_kba.csv")
```
...
```{r}
# st_write(wood_ducks_kba,"data/kba/wood_ducks_kba.shp", driver = "ESRI Shapefile", delete_layer = TRUE)
```

# 2.2 Priority Places {#2.2_PP}
...
Navigate to [Priority Places - Open Government Portal](https://open.canada.ca/data/en/dataset/91219d24-e877-4c8a-8bd2-b2b662e573e0). Scroll down to the **Data and Resources** section and select the Priority Places file labeled `English`, `dataset`, and `FGDB/GDB` to download it. Move this file to your `priority_places` subdirectory.

First, lets create a path to our downloaded Priority Place file.

```{r}
gdb_path <- "data/priority_places/PriorityPlaces.gdb"
```
...
```{r}
# st_write(wood_ducks_longpoint,"data/priority_places/wood_ducks_longpoint.shp", driver = "ESRI Shapefile", delete_layer = TRUE)
```
